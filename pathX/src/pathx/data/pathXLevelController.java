 ...
 */
public class pathXLevelController implements MouseListener, MouseMotionListener
{
     ...
     */
    public pathXLevelController(pathXDataModel initModel)
    {
    ...
    public void mousePressed(MouseEvent me)
    {
        ...
        pathXMiniGame game = (pathXMiniGame)model.getMiniGame();
            
        if(game.isCurrentScreenState(GAMEPLAY_SCREEN_STATE)){
           pathXIntersection i = model.findIntersectionAtCanvasLocation(canvasX, canvasY);
           if (i != null)
            {
                // MAKE THIS THE SELECTED INTERSECTION
                model.setSelectedIntersection(i);
                model.getPlayer().setTarget(canvasX, canvasY);
                model.movePlayer(canvasX, canvasY);
            ...
            pathXRoad r = model.selectRoadAtCanvasLocation(canvasX, canvasY);
            if (r != null)
            {
   ...
   //         model.switchEditMode(PXLE_EditMode.INTERSECTION_SELECTED);
    ...
    {
        // RIGHT MOUSE BUTTO0 N IS TO TOGGLE OPEN/CLOSE INTERSECTION
